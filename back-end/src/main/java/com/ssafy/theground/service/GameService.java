...
public class GameService {
	...
	private final String[] pitchTypes = { "fourSeam", "slider", "sinker", "changeUp", "curve", "cutter", "knuckleCurve",
			"splitter", "twoSeam", "knuckleball", "eephus", "screwball" };
		...
		if (byUserUid.isPresent()) {
			...
			if (matchHomeFlag) {
				...
				homeTeam.setStartingPitcher(pitcherRepository
						.findByPitcherSeq(byUserSeq.getTeamSettingNextSp().longValue()).getPitcherName());
	...
	public Map<String, Object> battingSimulate(String uid) {
		...
		if (log1stBase != null)
			base1Name = hitterRepository.findByHitterSeq(log1stBase).getHitterName();
		Long log2ndBase = log.getLog2ndBase();
		String base2Name = null;
		if (log2ndBase != null)
			base2Name = hitterRepository.findByHitterSeq(log2ndBase).getHitterName();
		Long log3rdBase = log.getLog3rdBase();
		String base3Name = null;
		if (log3rdBase != null)
			base3Name = hitterRepository.findByHitterSeq(log3rdBase).getHitterName();
		...
		if (inning == 9 && half.equals("Bottom")) {
			if (scoreboard.getScoreboardHomeScore() > scoreboard.getScoreboardAwayScore())
				inning = -1;
		}
